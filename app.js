...
function renderTrendChart() {
    ...
    if (sessions.length < 1) {
        container.innerHTML = `
            <div class="empty-trend-state">
                ...
                <span>Add sessions to plot trend history</span>
            </div>
        `;
        return;
    }
    ...
    // Grid Lines (Horizontal: 0%, 50%, 100%)
    let gridLinesSvg = "";
    [0, 50, 100].forEach(val => {
        ...
        gridLinesSvg += `
            ...
            <text class="chart-label-text" x="${paddingLeft - 6}" y="${y + 3}" text-anchor="end">${val}%</text>
        `;
    });
    ...
    sessions.forEach((session, i) => {
        ...
        let x = paddingLeft;
        if (len > 1) {
            x = paddingLeft + (i / (len - 1)) * graphWidth;
        } else {
            x = paddingLeft + graphWidth / 2; // Center singular point
        }
        
        // Calculate Y coordinate (inverse because Y increases downwards in SVG)
        const val = rates[i];
        const y = paddingTop + graphHeight - (val / 100) * graphHeight;
        
        points.push({ x, y, val, session });
        
        if (i === 0) {
            pathD += `M ${x} ${y}`;
            areaD += `M ${x} ${paddingTop + graphHeight} L ${x} ${y}`;
        } else {
            pathD += ` L ${x} ${y}`;
            areaD += ` L ${x} ${y}`;
        }
        ...
        if (i === len - 1) {
            areaD += ` L ${x} ${paddingTop + graphHeight} Z`;
        }
    });
    ...
    points.forEach((pt) => {
        chartElements += `
            <circle class="chart-point" cx="${pt.x}" cy="${pt.y}" r="4.5">
                <title>${pt.session}: ${pt.val}%</title>
            </circle>
        `;
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Redraw SVG chart on resize/rotate
